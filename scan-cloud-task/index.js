...
function getInput(what, use) {
    const value = tl.getInput(what); // the second parameter enforces required
    ...
    for (const ch of '<>|`$;&') {
        if (value.indexOf(ch) !== -1) {
            tl.setResult(tl.TaskResult.Failed, `the value of ${what} contains a forbidden character: ${ch}`);
            return undefined;
        }
    }
    return value;
}
function getVariable(what) {
    const value = tl.getVariable(what);
    if (value === undefined || value.length === 0) {
...
async function run() {
    try {
        ...
        let RL_VERBOSE = getInput('RL_VERBOSE', '0');
        if (RL_VERBOSE !== '0') {
            RL_VERBOSE = '1';
        }
        let RL_FORCE = getInput('RL_FORCE', '0');
        if (RL_FORCE !== '0') {
            RL_FORCE = '1';
        }
        const RL_DIFF_WITH = getInput('RL_DIFF_WITH', '');
        const RLSECURE_PROXY_SERVER = getInput('RLSECURE_PROXY_SERVER', '');
        const RLSECURE_PROXY_PORT = getInput('RLSECURE_PROXY_PORT', '');
        const RLSECURE_PROXY_USER = getInput('RLSECURE_PROXY_USER', '');
        // passwords may actually containg problematic chars
        const RLSECURE_PROXY_PASSWORD = tl.getInput('RLSECURE_PROXY_PASSWORD', false);
        // get input parameters RLPORTAL_SERVER, RLPORTAL_ORG, RLPORTAL_GROUP, RL_PACKAGE_URL
        const RLPORTAL_SERVER = getInput('RLPORTAL_SERVER');
        if (RLPORTAL_SERVER === undefined) {
            return;
        }
        const RLPORTAL_ORG = getInput('RLPORTAL_ORG');
        if (RLPORTAL_ORG === undefined) {
            return;
        }
        const RLPORTAL_GROUP = getInput('RLPORTAL_GROUP');
        if (RLPORTAL_GROUP === undefined) {
            return;
        }
        const RL_PACKAGE_URL = getInput('RL_PACKAGE_URL');
        if (RL_PACKAGE_URL === undefined) {
            return;
        }
        // get input parameters MY_ARTIFACT_TO_SCAN, REPORT_PATH, BUILD_PATH
        const MY_ARTIFACT_TO_SCAN = getInput('MY_ARTIFACT_TO_SCAN');
        if (MY_ARTIFACT_TO_SCAN === undefined) {
            return;
        }
        const REPORT_PATH = getInput('REPORT_PATH');
        if (REPORT_PATH === undefined) {
            return;
        }
        const BUILD_PATH = getInput('BUILD_PATH');
...
async function testBash(command) {
    try {
